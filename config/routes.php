...
return static function (Router $router): void {
    ...
    $router->get('/documents/{documentId:\d+}/download', 'DocumentController@download', ['roles' => ['patient', 'doctor', 'hospital_admin', 'paramedic', 'system_admin']]);
    $router->get('/reports/injury/{incidentId:\d+}', 'DocumentController@injuryReport', ['roles' => ['patient', 'doctor', 'hospital_admin', 'paramedic', 'system_admin']]);
    ...
    $router->get('/hospital/dashboard', 'HospitalController@dashboard', ['roles' => ['hospital_admin', 'doctor']]);
    $router->get('/api/hospital/incidents', 'HospitalController@incidents', ['roles' => ['hospital_admin', 'doctor']]);
    $router->post('/hospital/incidents/{incidentId:\d+}/assign', 'HospitalController@assignAmbulance', ['roles' => ['hospital_admin']]);

    $router->get('/ambulance/dashboard', 'AmbulanceController@dashboard', ['roles' => ['paramedic']]);
    ...
    $router->post('/doctor/cases/{caseId:\d+}/documents', 'DoctorController@uploadDocument', ['roles' => ['doctor']]);
